      ...
      ${reverse ? styles.boxReverseClass : styles.boxClass} w-11/12 sm:w-full minmd:w-3/4`}>
        ...
        ${reverse ? styles.textRight : styles.textLeft}`}>
          ...
          {showBtn && (<Button
            ...
            link="https://expo.dev/@parasta/react-native-app?serviceType=classic&distribution=expo-go"
          />)}
        </div>
     

      <div className={`flex-1 ${styles.flexCenter} p-8 sm:px-0`}>
        <img src={mockupImg} alt="mockup" className={` ${reverse ? 'fadeLeftMinifadeRightMini' : 'fadeRightMini'} ${styles.sectionImg}`} />
      </div>
      </div>
    </div>
  )
